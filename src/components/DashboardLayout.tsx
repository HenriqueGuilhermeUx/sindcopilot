...
const items = [
  ...
  [Users, "Fornecedores", "/fornecedores"],
  [UserPlus, "Ajudantes", "/ajudantes"],
] as const;

const lower = [
  [MessageSquare, "Assistente IA", "/assistente"],
  [Crown, "Planos", "/planos"],
  [User, "Meu Perfil", "/perfil"],
] as const;
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) {
    setTimeout(() => setLocation(`/login?redirect=${encodeURIComponent(location)}`), 0);
    return null;
  }
  ...
  const Sidebar = () => (
    <aside className="flex h-full w-72 flex-col bg-slate-950 text-slate-100">
      <div className="flex h-16 items-center gap-3 border-b border-white/10 px-5">
        <div className="grid h-9 w-9 place-items-center rounded-xl bg-cyan-500/15">
      ...
      <div className="border-t border-white/10 p-4 text-xs text-slate-400">
        Seus dados permanecem separados por conta e condomínio.
      </div>
    </aside>
  );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-background">
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 flex h-16 items-center border-b bg-background/95 px-4 backdrop-blur sm:px-6">
          ...
          {fieldMode && (
            <div className="ml-2 flex items-center gap-2 text-sm font-semibold lg:ml-0">
              <ClipboardCheck className="h-4 w-4 text-cyan-600" /> Modo Visita
            </div>
          )}
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 rounded-full p-1 hover:bg-muted">
                  ...
                  <span className="hidden text-left text-sm sm:block">
                    <span className="block font-medium">{user.name || "Síndico"}</span>
                    <span className="block max-w-44 truncate text-xs text-muted-foreground">{user.email}</span>
                  </span>
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuItem onSelect={() => nav("/perfil")}>
                  <User className="mr-2 h-4 w-4" /> Meu perfil
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={async () => { await signOut(); setLocation("/"); }}>
        ...
        <main className={cn("mx-auto max-w-7xl", fieldMode ? "p-0 sm:p-4 lg:p-6" : "p-4 sm:p-6 lg:p-8")}>
          {children}
        </main>
      </div>
